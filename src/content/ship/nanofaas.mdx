...
I wanted to better understand how JVM-based serverless platforms work under the hood—especially the trade-offs around performance, concurrency, startup behavior, and isolation.

Rather than relying only on documentation, I built a small Function-as-a-Service (FaaS) platform from scratch. The goal wasn’t to create a production-ready system, but to develop a concrete mental model for how modern FaaS runtimes—particularly JVM-based ones—are designed and why their constraints exist.

The result is **NanoFaaS**: a minimal, local FaaS that supports Python and Java functions, container-based execution, and in-process JVM execution using Vert.x.

---

## Overview

NanoFaaS is intentionally small, but complete end-to-end:
...
Everything runs locally using Docker Compose to make experimentation easy and repeatable.

---

## Execution Models

While building NanoFaaS, I explored several common FaaS runtime approaches:
...
NanoFaaS implements the first model and a simplified version of the third. The JVM runtime makes the trade-off especially visible: container-per-invocation is simple, but poorly suited to JVM startup characteristics.
...
This model produces highly predictable latency and throughput—but only when blocking operations are avoided. Violating that contract has immediate and visible consequences.
