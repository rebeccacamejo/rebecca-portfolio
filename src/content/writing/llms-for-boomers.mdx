...
At the heart of every LLM is a simple fact: **computers don’t understand words; they understand numbers.** Before any model can learn patterns in language, text has to be **broken into tokens** and mapped to **IDs** (numbers).  
...
💡 Fun Fact: ChatGPT pro version currently has a 128k token context size.
...
In the next post (*LLMs for Boomers, Part 2*), we’ll finally step into the model itself and see how embeddings and attention layers take these token IDs and turn them into language understanding through word embeddings.  
